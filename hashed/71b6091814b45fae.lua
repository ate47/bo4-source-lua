-- Generated by CoDLuaDecompiler
-- Version: 2.4.2
-- Date: 20-02-2025 19:35
require( "ui/uieditor/widgets/blackmarket/bm_goldbarmed" ) --[[ @ 0]]
require( "x64:394acc3cb318e13b" ) --[[ @ 0]]
require( "ui/uieditor/widgets/cac/cac_lock" ) --[[ @ 0]]
require( "x64:18ffe7c4e01e7052" ) --[[ @ 0]]
require( "x64:55f8cf56752aa71b" ) --[[ @ 0]]

CoD.StickerSetBonusDecal_Internal = InheritFrom( LUI.UIElement ) --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.__defaultWidth = 144 --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.__defaultHeight = 144 --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.new = function ( f1_arg0, f1_arg1, f1_arg2, f1_arg3, f1_arg4, f1_arg5, f1_arg6, f1_arg7, f1_arg8, f1_arg9 )
	local self = LUI.UIElement.new( f1_arg2, f1_arg3, f1_arg4, f1_arg5, f1_arg6, f1_arg7, f1_arg8, f1_arg9 ) --[[ @ 0]]
	self:setClass( CoD.StickerSetBonusDecal_Internal ) --[[ @ 0]]
	self.id = "StickerSetBonusDecal_Internal" --[[ @ 0]]
	self.soundSet = "default" --[[ @ 0]]
	self.onlyChildrenFocusable = true --[[ @ 0]]
	self.anyChildUsesUpdateState = true --[[ @ 0]]
	f1_arg0:addElementToPendingUpdateStateList( self ) --[[ @ 0]]
	
	local NoiseTiledBacking = LUI.UIImage.new( 0, 1, 0, 0, 0, 1, 0, 0 ) --[[ @ 0]]
	NoiseTiledBacking:setRGB( 1, 0.78, 0 ) --[[ @ 0]]
	NoiseTiledBacking:setAlpha( 0.8 ) --[[ @ 0]]
	NoiseTiledBacking:setImage( RegisterImage( @"uie_ui_menu_specialist_hub_repeat_bg" ) ) --[[ @ 0]]
	NoiseTiledBacking:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_16CBE95C250C6D15" ) ) --[[ @ 0]]
	NoiseTiledBacking:setShaderVector( 0, 0, 0, 0, 0 ) --[[ @ 0]]
	NoiseTiledBacking:setupNineSliceShader( 196, 88 ) --[[ @ 0]]
	self:addElement( NoiseTiledBacking ) --[[ @ 0]]
	self.NoiseTiledBacking = NoiseTiledBacking --[[ @ 0]]
	
	local BGAdd = LUI.UIImage.new( 0, 1, 0, 0, 0, 1, 0, 0 ) --[[ @ 0]]
	BGAdd:setRGB( 0.77, 0.78, 0 ) --[[ @ 0]]
	BGAdd:setAlpha( 0.02 ) --[[ @ 0]]
	BGAdd:setMaterial( LUI.UIImage.GetCachedMaterial( @"ui_add" ) ) --[[ @ 0]]
	self:addElement( BGAdd ) --[[ @ 0]]
	self.BGAdd = BGAdd --[[ @ 0]]
	
	local drawDecal = LUI.UIElement.new( 0, 1, 6, -6, 0, 1, 6, -6 ) --[[ @ 0]]
	drawDecal:setRGB( 0.92, 0.92, 0.92 ) --[[ @ 0]]
	drawDecal:linkToElementModel( self, "iconID", true, function ( model )
		local f2_local0 = model:get() --[[ @ 0]]
		if f2_local0 ~= nil then
			drawDecal:setupDrawDecalById( f2_local0 ) --[[ @ 0]]
		end
	end ) --[[ @ 0]]
	self:addElement( drawDecal ) --[[ @ 0]]
	self.drawDecal = drawDecal --[[ @ 0]]
	
	local CommonButtonOutline = CoD.CommonButtonOutline.new( f1_arg0, f1_arg1, 0, 1, 0, 0, 0, 1, 0, 0 ) --[[ @ 0]]
	CommonButtonOutline.Lines:setAlpha( 0 ) --[[ @ 0]]
	self:addElement( CommonButtonOutline ) --[[ @ 0]]
	self.CommonButtonOutline = CommonButtonOutline --[[ @ 0]]
	
	local FrameBorder = LUI.UIImage.new( 0, 1, -1, 1, 0, 1, -2, 2 ) --[[ @ 0]]
	FrameBorder:setRGB( 1, 0.58, 0 ) --[[ @ 0]]
	FrameBorder:setImage( RegisterImage( @"uie_ui_menu_store_common_frame" ) ) --[[ @ 0]]
	FrameBorder:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_6DAB59B2CAE01851" ) ) --[[ @ 0]]
	FrameBorder:setShaderVector( 0, 0, 0, 0, 0 ) --[[ @ 0]]
	FrameBorder:setShaderVector( 1, 1, 0, 0, 0 ) --[[ @ 0]]
	FrameBorder:setupNineSliceShader( 12, 12 ) --[[ @ 0]]
	self:addElement( FrameBorder ) --[[ @ 0]]
	self.FrameBorder = FrameBorder --[[ @ 0]]
	
	local BGGoldBar = CoD.BM_GoldBarMed.new( f1_arg0, f1_arg1, 0, 1, 0, 0, 0.5, 0.5, -15, 15 ) --[[ @ 0]]
	BGGoldBar:setAlpha( 0 ) --[[ @ 0]]
	self:addElement( BGGoldBar ) --[[ @ 0]]
	self.BGGoldBar = BGGoldBar --[[ @ 0]]
	
	local equippedIcon = LUI.UIImage.new( 1, 1, -27, -6, 0, 0, 6, 27 ) --[[ @ 0]]
	equippedIcon:setRGB( 0.56, 0.68, 0.27 ) --[[ @ 0]]
	equippedIcon:setAlpha( 0 ) --[[ @ 0]]
	equippedIcon:setImage( RegisterImage( @"hash_1B0337CBFC61A7A2" ) ) --[[ @ 0]]
	self:addElement( equippedIcon ) --[[ @ 0]]
	self.equippedIcon = equippedIcon --[[ @ 0]]
	
	local BreadcrumbNew = CoD.ItemNewBreadcrumbWidget.new( f1_arg0, f1_arg1, 0.5, 0.5, -78, 78, 0.5, 0.5, -18, 18 ) --[[ @ 0]]
	BreadcrumbNew:setAlpha( 0 ) --[[ @ 0]]
	self:addElement( BreadcrumbNew ) --[[ @ 0]]
	self.BreadcrumbNew = BreadcrumbNew --[[ @ 0]]
	
	local caclock = CoD.cac_lock.new( f1_arg0, f1_arg1, 0.5, 0.5, -24, 24, 0.5, 0.5, -24, 24 ) --[[ @ 0]]
	caclock:setAlpha( 0 ) --[[ @ 0]]
	caclock:setScale( 0.6, 0.6 ) --[[ @ 0]]
	self:addElement( caclock ) --[[ @ 0]]
	self.caclock = caclock --[[ @ 0]]
	
	local LockIcon = CoD.BM_Lock.new( f1_arg0, f1_arg1, 0.5, 0.5, -24, 24, 0.5, 0.5, -24, 24 ) --[[ @ 0]]
	LockIcon:setAlpha( 0 ) --[[ @ 0]]
	LockIcon:setScale( 0.6, 0.6 ) --[[ @ 0]]
	self:addElement( LockIcon ) --[[ @ 0]]
	self.LockIcon = LockIcon --[[ @ 0]]
	
	self:mergeStateConditions( {
		{
			stateName = "Unavailable",
			condition = function ( menu, element, event )
				return CoD.CraftUtility.IsCraftClassified( element, f1_arg1 )
			end
		},
		{
			stateName = "Locked",
			condition = function ( menu, element, event )
				return not CoD.ModelUtility.IsSelfModelValueTrue( element, f1_arg1, "owned" )
			end
		},
		{
			stateName = "New",
			condition = function ( menu, element, event )
				return CoD.BreadcrumbUtility.IsDecalStickerIconNew( menu, element, f1_arg1 )
			end
		}
	} ) --[[ @ 0]]
	self:linkToElementModel( self, "owned", true, function ( model )
		f1_arg0:updateElementState( self, {
			name = "model_validation",
			menu = f1_arg0,
			controller = f1_arg1,
			modelValue = model:get(),
			modelName = "owned"
		} ) --[[ @ 0]]
	end ) --[[ @ 0]]
	self:linkToElementModel( self, "available", true, function ( model )
		f1_arg0:updateElementState( self, {
			name = "model_validation",
			menu = f1_arg0,
			controller = f1_arg1,
			modelValue = model:get(),
			modelName = "available"
		} ) --[[ @ 0]]
	end ) --[[ @ 0]]
	self:linkToElementModel( self, "iconID", true, function ( model )
		f1_arg0:updateElementState( self, {
			name = "model_validation",
			menu = f1_arg0,
			controller = f1_arg1,
			modelValue = model:get(),
			modelName = "iconID"
		} ) --[[ @ 0]]
	end ) --[[ @ 0]]
	CommonButtonOutline.id = "CommonButtonOutline" --[[ @ 0]]
	LUI.OverrideFunction_CallOriginalSecond( self, "close", self.__onClose ) --[[ @ 0]]
	
	if PostLoadFunc then
		PostLoadFunc( self, f1_arg1, f1_arg0 ) --[[ @ 0]]
	end
	
	return self
end
 --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.__resetProperties = function ( f9_arg0 )
	f9_arg0.drawDecal:completeAnimation() --[[ @ 0]]
	f9_arg0.BGGoldBar:completeAnimation() --[[ @ 0]]
	f9_arg0.LockIcon:completeAnimation() --[[ @ 0]]
	f9_arg0.BreadcrumbNew:completeAnimation() --[[ @ 0]]
	f9_arg0.drawDecal:setRGB( 0.92, 0.92, 0.92 ) --[[ @ 0]]
	f9_arg0.drawDecal:setAlpha( 1 ) --[[ @ 0]]
	f9_arg0.BGGoldBar:setAlpha( 0 ) --[[ @ 0]]
	f9_arg0.LockIcon:setAlpha( 0 ) --[[ @ 0]]
	f9_arg0.BreadcrumbNew:setAlpha( 0 ) --[[ @ 0]]
end
 --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.__clipsPerState = {
	DefaultState = {
		DefaultClip = function ( f10_arg0, f10_arg1 )
			f10_arg0:__resetProperties() --[[ @ 0]]
			f10_arg0:setupElementClipCounter( 1 ) --[[ @ 0]]
			f10_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f10_arg0.drawDecal:setRGB( 0.92, 0.92, 0.92 ) --[[ @ 0]]
			f10_arg0.clipFinished( f10_arg0.drawDecal ) --[[ @ 0]]
		end
	},
	Unavailable = {
		DefaultClip = function ( f11_arg0, f11_arg1 )
			f11_arg0:__resetProperties() --[[ @ 0]]
			f11_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f11_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f11_arg0.drawDecal:setAlpha( 0 ) --[[ @ 0]]
			f11_arg0.clipFinished( f11_arg0.drawDecal ) --[[ @ 0]]
			f11_arg0.BGGoldBar:completeAnimation() --[[ @ 0]]
			f11_arg0.BGGoldBar:setAlpha( 1 ) --[[ @ 0]]
			f11_arg0.clipFinished( f11_arg0.BGGoldBar ) --[[ @ 0]]
		end,
		ChildFocus = function ( f12_arg0, f12_arg1 )
			f12_arg0:__resetProperties() --[[ @ 0]]
			f12_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f12_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f12_arg0.drawDecal:setAlpha( 0 ) --[[ @ 0]]
			f12_arg0.clipFinished( f12_arg0.drawDecal ) --[[ @ 0]]
			f12_arg0.BGGoldBar:completeAnimation() --[[ @ 0]]
			f12_arg0.BGGoldBar:setAlpha( 1 ) --[[ @ 0]]
			f12_arg0.clipFinished( f12_arg0.BGGoldBar ) --[[ @ 0]]
		end,
		GainChildFocus = function ( f13_arg0, f13_arg1 )
			f13_arg0:__resetProperties() --[[ @ 0]]
			f13_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f13_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f13_arg0.drawDecal:setAlpha( 0 ) --[[ @ 0]]
			f13_arg0.clipFinished( f13_arg0.drawDecal ) --[[ @ 0]]
			f13_arg0.BGGoldBar:completeAnimation() --[[ @ 0]]
			f13_arg0.BGGoldBar:setAlpha( 1 ) --[[ @ 0]]
			f13_arg0.clipFinished( f13_arg0.BGGoldBar ) --[[ @ 0]]
		end,
		LoseChildFocus = function ( f14_arg0, f14_arg1 )
			f14_arg0:__resetProperties() --[[ @ 0]]
			f14_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f14_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f14_arg0.drawDecal:setAlpha( 0 ) --[[ @ 0]]
			f14_arg0.clipFinished( f14_arg0.drawDecal ) --[[ @ 0]]
			f14_arg0.BGGoldBar:completeAnimation() --[[ @ 0]]
			f14_arg0.BGGoldBar:setAlpha( 1 ) --[[ @ 0]]
			f14_arg0.clipFinished( f14_arg0.BGGoldBar ) --[[ @ 0]]
		end
	},
	Locked = {
		DefaultClip = function ( f15_arg0, f15_arg1 )
			f15_arg0:__resetProperties() --[[ @ 0]]
			f15_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f15_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f15_arg0.drawDecal:setRGB( 0.29, 0.29, 0.29 ) --[[ @ 0]]
			f15_arg0.clipFinished( f15_arg0.drawDecal ) --[[ @ 0]]
			f15_arg0.LockIcon:completeAnimation() --[[ @ 0]]
			f15_arg0.LockIcon:setAlpha( 1 ) --[[ @ 0]]
			f15_arg0.clipFinished( f15_arg0.LockIcon ) --[[ @ 0]]
		end,
		ChildFocus = function ( f16_arg0, f16_arg1 )
			f16_arg0:__resetProperties() --[[ @ 0]]
			f16_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			f16_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f16_arg0.drawDecal:setRGB( 0.49, 0.49, 0.49 ) --[[ @ 0]]
			f16_arg0.clipFinished( f16_arg0.drawDecal ) --[[ @ 0]]
			f16_arg0.LockIcon:completeAnimation() --[[ @ 0]]
			f16_arg0.LockIcon:setAlpha( 1 ) --[[ @ 0]]
			f16_arg0.clipFinished( f16_arg0.LockIcon ) --[[ @ 0]]
		end,
		GainChildFocus = function ( f17_arg0, f17_arg1 )
			f17_arg0:__resetProperties() --[[ @ 0]]
			f17_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			local f17_local0 = function ( f18_arg0 )
				f17_arg0.drawDecal:beginAnimation( 200 ) --[[ @ 0]]
				f17_arg0.drawDecal:setRGB( 0.49, 0.49, 0.49 ) --[[ @ 0]]
				f17_arg0.drawDecal:registerEventHandler( "interrupted_keyframe", f17_arg0.clipInterrupted ) --[[ @ 0]]
				f17_arg0.drawDecal:registerEventHandler( "transition_complete_keyframe", f17_arg0.clipFinished ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f17_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f17_arg0.drawDecal:setRGB( 0.29, 0.29, 0.29 ) --[[ @ 0]]
			f17_local0( f17_arg0.drawDecal ) --[[ @ 0]]
			f17_arg0.LockIcon:completeAnimation() --[[ @ 0]]
			f17_arg0.LockIcon:setAlpha( 1 ) --[[ @ 0]]
			f17_arg0.clipFinished( f17_arg0.LockIcon ) --[[ @ 0]]
		end,
		LoseChildFocus = function ( f19_arg0, f19_arg1 )
			f19_arg0:__resetProperties() --[[ @ 0]]
			f19_arg0:setupElementClipCounter( 2 ) --[[ @ 0]]
			local f19_local0 = function ( f20_arg0 )
				f19_arg0.drawDecal:beginAnimation( 200 ) --[[ @ 0]]
				f19_arg0.drawDecal:setRGB( 0.29, 0.29, 0.29 ) --[[ @ 0]]
				f19_arg0.drawDecal:registerEventHandler( "interrupted_keyframe", f19_arg0.clipInterrupted ) --[[ @ 0]]
				f19_arg0.drawDecal:registerEventHandler( "transition_complete_keyframe", f19_arg0.clipFinished ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f19_arg0.drawDecal:completeAnimation() --[[ @ 0]]
			f19_arg0.drawDecal:setRGB( 0.49, 0.49, 0.49 ) --[[ @ 0]]
			f19_local0( f19_arg0.drawDecal ) --[[ @ 0]]
			f19_arg0.LockIcon:completeAnimation() --[[ @ 0]]
			f19_arg0.LockIcon:setAlpha( 1 ) --[[ @ 0]]
			f19_arg0.clipFinished( f19_arg0.LockIcon ) --[[ @ 0]]
		end
	},
	New = {
		DefaultClip = function ( f21_arg0, f21_arg1 )
			f21_arg0:__resetProperties() --[[ @ 0]]
			f21_arg0:setupElementClipCounter( 1 ) --[[ @ 0]]
			f21_arg0.BreadcrumbNew:completeAnimation() --[[ @ 0]]
			f21_arg0.BreadcrumbNew:setAlpha( 1 ) --[[ @ 0]]
			f21_arg0.clipFinished( f21_arg0.BreadcrumbNew ) --[[ @ 0]]
		end,
		ChildFocus = function ( f22_arg0, f22_arg1 )
			f22_arg0:__resetProperties() --[[ @ 0]]
			f22_arg0:setupElementClipCounter( 1 ) --[[ @ 0]]
			local f22_local0 = function ( f23_arg0 )
				local f23_local0 = function ( f24_arg0 )
					f24_arg0:beginAnimation( 200 ) --[[ @ 0]]
					f24_arg0:setAlpha( 0 ) --[[ @ 0]]
					f24_arg0:registerEventHandler( "transition_complete_keyframe", f22_arg0.clipFinished ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f22_arg0.BreadcrumbNew:beginAnimation( 400 ) --[[ @ 0]]
				f22_arg0.BreadcrumbNew:registerEventHandler( "interrupted_keyframe", f22_arg0.clipInterrupted ) --[[ @ 0]]
				f22_arg0.BreadcrumbNew:registerEventHandler( "transition_complete_keyframe", f23_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f22_arg0.BreadcrumbNew:completeAnimation() --[[ @ 0]]
			f22_arg0.BreadcrumbNew:setAlpha( 1 ) --[[ @ 0]]
			f22_local0( f22_arg0.BreadcrumbNew ) --[[ @ 0]]
		end,
		GainChildFocus = function ( f25_arg0, f25_arg1 )
			f25_arg0:__resetProperties() --[[ @ 0]]
			f25_arg0:setupElementClipCounter( 1 ) --[[ @ 0]]
			f25_arg0.BreadcrumbNew:completeAnimation() --[[ @ 0]]
			f25_arg0.BreadcrumbNew:setAlpha( 1 ) --[[ @ 0]]
			f25_arg0.clipFinished( f25_arg0.BreadcrumbNew ) --[[ @ 0]]
		end,
		LoseChildFocus = function ( f26_arg0, f26_arg1 )
			f26_arg0:__resetProperties() --[[ @ 0]]
			f26_arg0:setupElementClipCounter( 1 ) --[[ @ 0]]
			f26_arg0.BreadcrumbNew:completeAnimation() --[[ @ 0]]
			f26_arg0.BreadcrumbNew:setAlpha( 1 ) --[[ @ 0]]
			f26_arg0.clipFinished( f26_arg0.BreadcrumbNew ) --[[ @ 0]]
		end
	}
} --[[ @ 0]]
CoD.StickerSetBonusDecal_Internal.__onClose = function ( f27_arg0 )
	f27_arg0.drawDecal:close() --[[ @ 0]]
	f27_arg0.CommonButtonOutline:close() --[[ @ 0]]
	f27_arg0.BGGoldBar:close() --[[ @ 0]]
	f27_arg0.BreadcrumbNew:close() --[[ @ 0]]
	f27_arg0.caclock:close() --[[ @ 0]]
	f27_arg0.LockIcon:close() --[[ @ 0]]
end
 --[[ @ 0]]
