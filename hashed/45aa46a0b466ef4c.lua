-- Generated by CoDLuaDecompiler
-- Version: 2.4.2
-- Date: 20-02-2025 19:34
CoD.TabletIcePickGadget_BgElementBinary = InheritFrom( LUI.UIElement ) --[[ @ 0]]
CoD.TabletIcePickGadget_BgElementBinary.__defaultWidth = 656 --[[ @ 0]]
CoD.TabletIcePickGadget_BgElementBinary.__defaultHeight = 120 --[[ @ 0]]
CoD.TabletIcePickGadget_BgElementBinary.new = function ( f1_arg0, f1_arg1, f1_arg2, f1_arg3, f1_arg4, f1_arg5, f1_arg6, f1_arg7, f1_arg8, f1_arg9 )
	local self = LUI.UIElement.new( f1_arg2, f1_arg3, f1_arg4, f1_arg5, f1_arg6, f1_arg7, f1_arg8, f1_arg9 ) --[[ @ 0]]
	self:setClass( CoD.TabletIcePickGadget_BgElementBinary ) --[[ @ 0]]
	self.id = "TabletIcePickGadget_BgElementBinary" --[[ @ 0]]
	self.soundSet = "default" --[[ @ 0]]
	f1_arg0:addElementToPendingUpdateStateList( self ) --[[ @ 0]]
	
	local binary05 = LUI.UIImage.new( 0, 0, 30, 342, 0, 0, 14, 134 ) --[[ @ 0]]
	binary05:setAlpha( 0 ) --[[ @ 0]]
	binary05:setImage( RegisterImage( @"hash_2D5F0B06802219AA" ) ) --[[ @ 0]]
	binary05:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_31CC85D0A86303B0" ) ) --[[ @ 0]]
	binary05:setShaderVector( 0, 5, 0, 0, 0 ) --[[ @ 0]]
	self:addElement( binary05 ) --[[ @ 0]]
	self.binary05 = binary05 --[[ @ 0]]
	
	local binary04 = LUI.UIImage.new( 0, 0, 0, 632, 0, 0, 15, 135 ) --[[ @ 0]]
	binary04:setAlpha( 0 ) --[[ @ 0]]
	binary04:setImage( RegisterImage( @"hash_2D5F0C0680221B5D" ) ) --[[ @ 0]]
	binary04:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_31CC85D0A86303B0" ) ) --[[ @ 0]]
	binary04:setShaderVector( 0, 5, 0, 0, 0 ) --[[ @ 0]]
	self:addElement( binary04 ) --[[ @ 0]]
	self.binary04 = binary04 --[[ @ 0]]
	
	local binary03 = LUI.UIImage.new( 0, 0, 0, 336, 0, 0, -14, 106 ) --[[ @ 0]]
	binary03:setAlpha( 0 ) --[[ @ 0]]
	binary03:setImage( RegisterImage( @"hash_2D5F050680220F78" ) ) --[[ @ 0]]
	binary03:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_31CC85D0A86303B0" ) ) --[[ @ 0]]
	binary03:setShaderVector( 0, 5, 0, 0, 0 ) --[[ @ 0]]
	self:addElement( binary03 ) --[[ @ 0]]
	self.binary03 = binary03 --[[ @ 0]]
	
	local binary02 = LUI.UIImage.new( 0, 0, 0, 656, 0, 0, 0, 120 ) --[[ @ 0]]
	binary02:setAlpha( 0 ) --[[ @ 0]]
	binary02:setImage( RegisterImage( @"hash_2D5F06068022112B" ) ) --[[ @ 0]]
	binary02:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_31CC85D0A86303B0" ) ) --[[ @ 0]]
	binary02:setShaderVector( 0, 5, 0, 0, 0 ) --[[ @ 0]]
	self:addElement( binary02 ) --[[ @ 0]]
	self.binary02 = binary02 --[[ @ 0]]
	
	local binary01 = LUI.UIImage.new( 0, 0, 0, 304, 0, 0, 0, 120 ) --[[ @ 0]]
	binary01:setImage( RegisterImage( @"hash_2D5F080680221491" ) ) --[[ @ 0]]
	binary01:setMaterial( LUI.UIImage.GetCachedMaterial( @"hash_31CC85D0A86303B0" ) ) --[[ @ 0]]
	binary01:setShaderVector( 0, 5, 0, 0, 0 ) --[[ @ 0]]
	self:addElement( binary01 ) --[[ @ 0]]
	self.binary01 = binary01 --[[ @ 0]]
	
	if PostLoadFunc then
		PostLoadFunc( self, f1_arg1, f1_arg0 ) --[[ @ 0]]
	end
	
	return self
end
 --[[ @ 0]]
CoD.TabletIcePickGadget_BgElementBinary.__resetProperties = function ( f2_arg0 )
	f2_arg0.binary02:completeAnimation() --[[ @ 0]]
	f2_arg0.binary03:completeAnimation() --[[ @ 0]]
	f2_arg0.binary04:completeAnimation() --[[ @ 0]]
	f2_arg0.binary01:completeAnimation() --[[ @ 0]]
	f2_arg0.binary05:completeAnimation() --[[ @ 0]]
	f2_arg0.binary02:setAlpha( 0 ) --[[ @ 0]]
	f2_arg0.binary03:setAlpha( 0 ) --[[ @ 0]]
	f2_arg0.binary04:setAlpha( 0 ) --[[ @ 0]]
	f2_arg0.binary01:setAlpha( 1 ) --[[ @ 0]]
	f2_arg0.binary05:setAlpha( 0 ) --[[ @ 0]]
end
 --[[ @ 0]]
CoD.TabletIcePickGadget_BgElementBinary.__clipsPerState = {
	DefaultState = {
		DefaultClip = function ( f3_arg0, f3_arg1 )
			f3_arg0:__resetProperties() --[[ @ 0]]
			f3_arg0:setupElementClipCounter( 5 ) --[[ @ 0]]
			local f3_local0 = function ( f4_arg0 )
				local f4_local0 = function ( f5_arg0 )
					local f5_local0 = function ( f6_arg0 )
						local f6_local0 = function ( f7_arg0 )
							local f7_local0 = function ( f8_arg0 )
								local f8_local0 = function ( f9_arg0 )
									local f9_local0 = function ( f10_arg0 )
										local f10_local0 = function ( f11_arg0 )
											f11_arg0:beginAnimation( 19 ) --[[ @ 0]]
											f11_arg0:setAlpha( 0 ) --[[ @ 0]]
											f11_arg0:registerEventHandler( "transition_complete_keyframe", f3_arg0.clipFinished ) --[[ @ 0]]
										end
										 --[[ @ 0]]
										f10_arg0:beginAnimation( 60 ) --[[ @ 0]]
										f10_arg0:registerEventHandler( "transition_complete_keyframe", f10_local0 ) --[[ @ 0]]
									end
									 --[[ @ 0]]
									f9_arg0:beginAnimation( 19 ) --[[ @ 0]]
									f9_arg0:setAlpha( 0.7 ) --[[ @ 0]]
									f9_arg0:registerEventHandler( "transition_complete_keyframe", f9_local0 ) --[[ @ 0]]
								end
								 --[[ @ 0]]
								f8_arg0:beginAnimation( 369 ) --[[ @ 0]]
								f8_arg0:registerEventHandler( "transition_complete_keyframe", f8_local0 ) --[[ @ 0]]
							end
							 --[[ @ 0]]
							f7_arg0:beginAnimation( 20 ) --[[ @ 0]]
							f7_arg0:setAlpha( 0 ) --[[ @ 0]]
							f7_arg0:registerEventHandler( "transition_complete_keyframe", f7_local0 ) --[[ @ 0]]
						end
						 --[[ @ 0]]
						f6_arg0:beginAnimation( 60 ) --[[ @ 0]]
						f6_arg0:registerEventHandler( "transition_complete_keyframe", f6_local0 ) --[[ @ 0]]
					end
					 --[[ @ 0]]
					f5_arg0:beginAnimation( 19 ) --[[ @ 0]]
					f5_arg0:setAlpha( 0.6 ) --[[ @ 0]]
					f5_arg0:registerEventHandler( "transition_complete_keyframe", f5_local0 ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f3_arg0.binary05:beginAnimation( 560 ) --[[ @ 0]]
				f3_arg0.binary05:registerEventHandler( "interrupted_keyframe", f3_arg0.clipInterrupted ) --[[ @ 0]]
				f3_arg0.binary05:registerEventHandler( "transition_complete_keyframe", f4_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f3_arg0.binary05:completeAnimation() --[[ @ 0]]
			f3_arg0.binary05:setAlpha( 0 ) --[[ @ 0]]
			f3_local0( f3_arg0.binary05 ) --[[ @ 0]]
			local f3_local1 = function ( f12_arg0 )
				local f12_local0 = function ( f13_arg0 )
					local f13_local0 = function ( f14_arg0 )
						local f14_local0 = function ( f15_arg0 )
							local f15_local0 = function ( f16_arg0 )
								local f16_local0 = function ( f17_arg0 )
									local f17_local0 = function ( f18_arg0 )
										local f18_local0 = function ( f19_arg0 )
											local f19_local0 = function ( f20_arg0 )
												f20_arg0:beginAnimation( 69 ) --[[ @ 0]]
												f20_arg0:registerEventHandler( "transition_complete_keyframe", f3_arg0.clipFinished ) --[[ @ 0]]
											end
											 --[[ @ 0]]
											f19_arg0:beginAnimation( 19 ) --[[ @ 0]]
											f19_arg0:setAlpha( 0 ) --[[ @ 0]]
											f19_arg0:registerEventHandler( "transition_complete_keyframe", f19_local0 ) --[[ @ 0]]
										end
										 --[[ @ 0]]
										f18_arg0:beginAnimation( 60 ) --[[ @ 0]]
										f18_arg0:registerEventHandler( "transition_complete_keyframe", f18_local0 ) --[[ @ 0]]
									end
									 --[[ @ 0]]
									f17_arg0:beginAnimation( 19 ) --[[ @ 0]]
									f17_arg0:setAlpha( 0.8 ) --[[ @ 0]]
									f17_arg0:registerEventHandler( "transition_complete_keyframe", f17_local0 ) --[[ @ 0]]
								end
								 --[[ @ 0]]
								f16_arg0:beginAnimation( 669 ) --[[ @ 0]]
								f16_arg0:registerEventHandler( "transition_complete_keyframe", f16_local0 ) --[[ @ 0]]
							end
							 --[[ @ 0]]
							f15_arg0:beginAnimation( 20 ) --[[ @ 0]]
							f15_arg0:setAlpha( 0 ) --[[ @ 0]]
							f15_arg0:registerEventHandler( "transition_complete_keyframe", f15_local0 ) --[[ @ 0]]
						end
						 --[[ @ 0]]
						f14_arg0:beginAnimation( 120 ) --[[ @ 0]]
						f14_arg0:registerEventHandler( "transition_complete_keyframe", f14_local0 ) --[[ @ 0]]
					end
					 --[[ @ 0]]
					f13_arg0:beginAnimation( 19 ) --[[ @ 0]]
					f13_arg0:setAlpha( 0.8 ) --[[ @ 0]]
					f13_arg0:registerEventHandler( "transition_complete_keyframe", f13_local0 ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f3_arg0.binary04:beginAnimation( 300 ) --[[ @ 0]]
				f3_arg0.binary04:registerEventHandler( "interrupted_keyframe", f3_arg0.clipInterrupted ) --[[ @ 0]]
				f3_arg0.binary04:registerEventHandler( "transition_complete_keyframe", f12_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f3_arg0.binary04:completeAnimation() --[[ @ 0]]
			f3_arg0.binary04:setAlpha( 0 ) --[[ @ 0]]
			f3_local1( f3_arg0.binary04 ) --[[ @ 0]]
			local f3_local2 = function ( f21_arg0 )
				local f21_local0 = function ( f22_arg0 )
					local f22_local0 = function ( f23_arg0 )
						local f23_local0 = function ( f24_arg0 )
							local f24_local0 = function ( f25_arg0 )
								local f25_local0 = function ( f26_arg0 )
									local f26_local0 = function ( f27_arg0 )
										local f27_local0 = function ( f28_arg0 )
											f28_arg0:beginAnimation( 19 ) --[[ @ 0]]
											f28_arg0:setAlpha( 0 ) --[[ @ 0]]
											f28_arg0:registerEventHandler( "transition_complete_keyframe", f3_arg0.clipFinished ) --[[ @ 0]]
										end
										 --[[ @ 0]]
										f27_arg0:beginAnimation( 130 ) --[[ @ 0]]
										f27_arg0:registerEventHandler( "transition_complete_keyframe", f27_local0 ) --[[ @ 0]]
									end
									 --[[ @ 0]]
									f26_arg0:beginAnimation( 19 ) --[[ @ 0]]
									f26_arg0:setAlpha( 0.8 ) --[[ @ 0]]
									f26_arg0:registerEventHandler( "transition_complete_keyframe", f26_local0 ) --[[ @ 0]]
								end
								 --[[ @ 0]]
								f25_arg0:beginAnimation( 360 ) --[[ @ 0]]
								f25_arg0:registerEventHandler( "transition_complete_keyframe", f25_local0 ) --[[ @ 0]]
							end
							 --[[ @ 0]]
							f24_arg0:beginAnimation( 20 ) --[[ @ 0]]
							f24_arg0:setAlpha( 0 ) --[[ @ 0]]
							f24_arg0:registerEventHandler( "transition_complete_keyframe", f24_local0 ) --[[ @ 0]]
						end
						 --[[ @ 0]]
						f23_arg0:beginAnimation( 60 ) --[[ @ 0]]
						f23_arg0:registerEventHandler( "transition_complete_keyframe", f23_local0 ) --[[ @ 0]]
					end
					 --[[ @ 0]]
					f22_arg0:beginAnimation( 19 ) --[[ @ 0]]
					f22_arg0:setAlpha( 0.8 ) --[[ @ 0]]
					f22_arg0:registerEventHandler( "transition_complete_keyframe", f22_local0 ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f3_arg0.binary03:beginAnimation( 200 ) --[[ @ 0]]
				f3_arg0.binary03:registerEventHandler( "interrupted_keyframe", f3_arg0.clipInterrupted ) --[[ @ 0]]
				f3_arg0.binary03:registerEventHandler( "transition_complete_keyframe", f21_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f3_arg0.binary03:completeAnimation() --[[ @ 0]]
			f3_arg0.binary03:setAlpha( 0 ) --[[ @ 0]]
			f3_local2( f3_arg0.binary03 ) --[[ @ 0]]
			local f3_local3 = function ( f29_arg0 )
				local f29_local0 = function ( f30_arg0 )
					local f30_local0 = function ( f31_arg0 )
						local f31_local0 = function ( f32_arg0 )
							local f32_local0 = function ( f33_arg0 )
								local f33_local0 = function ( f34_arg0 )
									local f34_local0 = function ( f35_arg0 )
										local f35_local0 = function ( f36_arg0 )
											local f36_local0 = function ( f37_arg0 )
												local f37_local0 = function ( f38_arg0 )
													local f38_local0 = function ( f39_arg0 )
														local f39_local0 = function ( f40_arg0 )
															f40_arg0:beginAnimation( 19 ) --[[ @ 0]]
															f40_arg0:setAlpha( 0 ) --[[ @ 0]]
															f40_arg0:registerEventHandler( "transition_complete_keyframe", f3_arg0.clipFinished ) --[[ @ 0]]
														end
														 --[[ @ 0]]
														f39_arg0:beginAnimation( 60 ) --[[ @ 0]]
														f39_arg0:registerEventHandler( "transition_complete_keyframe", f39_local0 ) --[[ @ 0]]
													end
													 --[[ @ 0]]
													f38_arg0:beginAnimation( 19 ) --[[ @ 0]]
													f38_arg0:setAlpha( 0.85 ) --[[ @ 0]]
													f38_arg0:registerEventHandler( "transition_complete_keyframe", f38_local0 ) --[[ @ 0]]
												end
												 --[[ @ 0]]
												f37_arg0:beginAnimation( 370 ) --[[ @ 0]]
												f37_arg0:registerEventHandler( "transition_complete_keyframe", f37_local0 ) --[[ @ 0]]
											end
											 --[[ @ 0]]
											f36_arg0:beginAnimation( 19 ) --[[ @ 0]]
											f36_arg0:setAlpha( 0 ) --[[ @ 0]]
											f36_arg0:registerEventHandler( "transition_complete_keyframe", f36_local0 ) --[[ @ 0]]
										end
										 --[[ @ 0]]
										f35_arg0:beginAnimation( 60 ) --[[ @ 0]]
										f35_arg0:registerEventHandler( "transition_complete_keyframe", f35_local0 ) --[[ @ 0]]
									end
									 --[[ @ 0]]
									f34_arg0:beginAnimation( 19 ) --[[ @ 0]]
									f34_arg0:setAlpha( 0.7 ) --[[ @ 0]]
									f34_arg0:registerEventHandler( "transition_complete_keyframe", f34_local0 ) --[[ @ 0]]
								end
								 --[[ @ 0]]
								f33_arg0:beginAnimation( 260 ) --[[ @ 0]]
								f33_arg0:registerEventHandler( "transition_complete_keyframe", f33_local0 ) --[[ @ 0]]
							end
							 --[[ @ 0]]
							f32_arg0:beginAnimation( 19 ) --[[ @ 0]]
							f32_arg0:setAlpha( 0 ) --[[ @ 0]]
							f32_arg0:registerEventHandler( "transition_complete_keyframe", f32_local0 ) --[[ @ 0]]
						end
						 --[[ @ 0]]
						f31_arg0:beginAnimation( 60 ) --[[ @ 0]]
						f31_arg0:registerEventHandler( "transition_complete_keyframe", f31_local0 ) --[[ @ 0]]
					end
					 --[[ @ 0]]
					f30_arg0:beginAnimation( 19 ) --[[ @ 0]]
					f30_arg0:setAlpha( 0.9 ) --[[ @ 0]]
					f30_arg0:registerEventHandler( "transition_complete_keyframe", f30_local0 ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f3_arg0.binary02:beginAnimation( 100 ) --[[ @ 0]]
				f3_arg0.binary02:registerEventHandler( "interrupted_keyframe", f3_arg0.clipInterrupted ) --[[ @ 0]]
				f3_arg0.binary02:registerEventHandler( "transition_complete_keyframe", f29_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f3_arg0.binary02:completeAnimation() --[[ @ 0]]
			f3_arg0.binary02:setAlpha( 0 ) --[[ @ 0]]
			f3_local3( f3_arg0.binary02 ) --[[ @ 0]]
			local f3_local4 = function ( f41_arg0 )
				local f41_local0 = function ( f42_arg0 )
					local f42_local0 = function ( f43_arg0 )
						local f43_local0 = function ( f44_arg0 )
							local f44_local0 = function ( f45_arg0 )
								local f45_local0 = function ( f46_arg0 )
									local f46_local0 = function ( f47_arg0 )
										f47_arg0:beginAnimation( 19 ) --[[ @ 0]]
										f47_arg0:setAlpha( 0 ) --[[ @ 0]]
										f47_arg0:registerEventHandler( "transition_complete_keyframe", f3_arg0.clipFinished ) --[[ @ 0]]
									end
									 --[[ @ 0]]
									f46_arg0:beginAnimation( 60 ) --[[ @ 0]]
									f46_arg0:registerEventHandler( "transition_complete_keyframe", f46_local0 ) --[[ @ 0]]
								end
								 --[[ @ 0]]
								f45_arg0:beginAnimation( 20 ) --[[ @ 0]]
								f45_arg0:setAlpha( 0.7 ) --[[ @ 0]]
								f45_arg0:registerEventHandler( "transition_complete_keyframe", f45_local0 ) --[[ @ 0]]
							end
							 --[[ @ 0]]
							f44_arg0:beginAnimation( 729 ) --[[ @ 0]]
							f44_arg0:registerEventHandler( "transition_complete_keyframe", f44_local0 ) --[[ @ 0]]
						end
						 --[[ @ 0]]
						f43_arg0:beginAnimation( 20 ) --[[ @ 0]]
						f43_arg0:setAlpha( 0 ) --[[ @ 0]]
						f43_arg0:registerEventHandler( "transition_complete_keyframe", f43_local0 ) --[[ @ 0]]
					end
					 --[[ @ 0]]
					f42_arg0:beginAnimation( 60 ) --[[ @ 0]]
					f42_arg0:registerEventHandler( "transition_complete_keyframe", f42_local0 ) --[[ @ 0]]
				end
				 --[[ @ 0]]
				f3_arg0.binary01:beginAnimation( 20 ) --[[ @ 0]]
				f3_arg0.binary01:setAlpha( 0.6 ) --[[ @ 0]]
				f3_arg0.binary01:registerEventHandler( "interrupted_keyframe", f3_arg0.clipInterrupted ) --[[ @ 0]]
				f3_arg0.binary01:registerEventHandler( "transition_complete_keyframe", f41_local0 ) --[[ @ 0]]
			end
			 --[[ @ 0]]
			f3_arg0.binary01:completeAnimation() --[[ @ 0]]
			f3_arg0.binary01:setAlpha( 0 ) --[[ @ 0]]
			f3_local4( f3_arg0.binary01 ) --[[ @ 0]]
			f3_arg0.nextClip = "DefaultClip" --[[ @ 0]]
		end
	}
} --[[ @ 0]]
