...
Lobby.RecentPlayers.ShouldAddPlayer = function ( f2_arg0, f2_arg1 )
	if f2_arg1.lobbyClientType ~= nil and (f2_arg1.lobbyClientType == Enum[@"LobbyClientType"][@"hash_2EA66E24E2A1A4D9"] or f2_arg1.lobbyClientType == Enum[@"LobbyClientType"][@"hash_45768EF8BA18333F"]) then
		return false
	elseif Engine[@"hash_60041DC2CF4139D7"]( f2_arg1.xuid ) then
		return false
...
Lobby.RecentPlayers.OnClientAdded = function ( f6_arg0 )
	if f6_arg0.lobbyModule == Enum[@"LobbyModule"][@"hash_40C46B73E8E18BA2"] then
		if Enum[@"eModes"][@"hash_3BF1DCC8138A9D39"] == Engine[@"CurrentSessionMode"]() then
			if f6_arg0.lobbyType == Enum[@"LobbyType"][@"hash_20A1647599284110"] then
				Lobby.RecentPlayers.AddToAllActiveControllers( Lobby.RecentPlayers.AddOneRecentPlayer, {
					xuid = f6_arg0.xuid,
					gamertag = f6_arg0.customData
				} ) --[[ @ 0]]
			end
		elseif (Engine[@"IsInGame"]() or not Lobby.RecentPlayers.Options.InGameOnly or f6_arg0.lobbyType == Enum[@"LobbyType"][@"hash_20A1647599284110"]) and not Engine[@"hash_65CB8E6B7FBBFFD5"]( f6_arg0.xuid ) then
			Lobby.RecentPlayers.AddToAllActiveControllers( Lobby.RecentPlayers.AddOneRecentPlayer, {
				xuid = f6_arg0.xuid,
				gamertag = f6_arg0.customData
			} ) --[[ @ 0]]
...
Lobby.RecentPlayers.OnMatchStart = function ( f7_arg0 )
	if Engine[@"CurrentSessionMode"]() ~= Enum[@"eModes"][@"hash_3BF1DCC8138A9D39"] then
		local f7_local0 = Engine[@"hash_4755D55B3813D249"]( f7_arg0.lobbyModule, f7_arg0.lobbyType ) --[[ @ 0]]
...
Lobby.RecentPlayers.OnWarzoneMatchStart = function ( f9_arg0 )
	local f9_local0 = Enum[@"LobbyModule"][@"hash_40C46B73E8E18BA2"] --[[ @ 0]]
	local f9_local1 = Enum[@"LobbyType"][@"hash_692676CF5B6FCD43"] --[[ @ 0]]
	...
	for f9_local11, f9_local12 in pairs( f9_local4 ) --[[ @ 0]] do
		if f9_local3 == f9_local12 then
			local f9_local10 = Engine[@"hash_7DB23F7EB606D71B"]( f9_local0, f9_local1, f9_local11 ) --[[ @ 0]]
...
Lobby.RecentPlayers.OnLocalClientObituary = function ( f10_arg0 )
	if Engine[@"CurrentSessionMode"]() == Enum[@"eModes"][@"hash_3BF1DCC8138A9D39"] and f10_arg0.meansofdeath ~= Enum[@"hash_752177B239BFECC1"][@"mod_downed"] then
