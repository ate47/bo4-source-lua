...
CoD.DoubleXPUtility.MainModeHasDoubleWeaponXP = function ( f8_arg0, f8_arg1, f8_arg2 )
	if f8_arg1 == Enum[@"LobbyMainMode"][@"hash_378C124999125C42"] then
		return false
	end
...
CoD.DoubleXPUtility.HasDoubleLootXP = function ( f10_arg0, f10_arg1 )
	...
	if f10_local0 == Enum[@"eModes"][@"hash_73723205FAE52C4A"] or CoD.StartMenuUtility.GetSessionModeFromLobby() == Enum[@"eModes"][@"hash_73723205FAE52C4A"] then
		return false
	else
		local f10_local2 --[[ @ 0]]
		if f10_local0 then
...
CoD.DoubleXPUtility.CurrentLobbyHasTierBoost = function ( f12_arg0 )
	...
	if f12_local0 == Enum[@"LobbyMainMode"][@"hash_379D01499920B292"] or f12_local0 == Enum[@"LobbyMainMode"][@"hash_6D5FBB8D74AC6D62"] then
		return false
	end
	...
	if IsPublicOnlineGame() then
		f12_local3 = Engine[@"hash_17B3B2B73B53EB34"]() --[[ @ 0]]
	end
...
CoD.DoubleXPUtility.HasDoubleNP = function ( f15_arg0, f15_arg1 )
	...
	if (f15_local0 == Enum[@"eModes"][@"hash_73723205FAE52C4A"] or CoD.StartMenuUtility.GetSessionModeFromLobby() == Enum[@"eModes"][@"hash_73723205FAE52C4A"]) and (Engine[@"hash_5F4F47F17B3BDCB3"]( f15_arg0, Engine[@"hash_2ACFE452C407B25B"]( Enum[@"eModes"][@"hash_73723205FAE52C4A"] ), Enum[@"eModes"][@"hash_73723205FAE52C4A"], f15_local1 ) or CoD.BaseUtility.IsDvarEnabled( "ui_fakeDoubleNP" )) then
		return true
	else
		return false
	end
...
CoD.DoubleXPUtility.CurrentLobbyHasDoubleNP = function ( f16_arg0 )
	if not CoD.ZombieUtility.IsDoubleNP( f16_arg0 ) then
		...
		return f16_local0.lobbyRoot.lobbyMainMode:get() == Enum[@"LobbyMainMode"][@"hash_379D01499920B292"]
	end
end
 --[[ @ 0]]
CoD.DoubleXPUtility.CurrentPlaylistHasDoubleXP = function ( f17_arg0 )
	local f17_local0 = LobbyData.GetCurrentMenuTarget() --[[ @ 0]]
	if not IsPublicOrLeagueGame() and (not f17_local0 or f17_local0[@"LobbyMode"] == Enum[@"LobbyMode"][@"hash_3F5EE25D311E5223"]) and not CoD.DirectorUtility.ShowDirectorPregame( f17_arg0 ) then
		return false
	else
		...
		if f17_local1[@"id"] == LobbyData.GetLobbyMenuIDByName( LuaEnum.UI.DIRECTOR_ONLINE_MP_TRAINING ) or IsSimulateCT() then
			...
			local f17_local3 = f17_local2 and Engine[@"hash_63ACB99DBAD24D55"]( f17_local2 ) --[[ @ 0]]
			...
			elseif CoD.BaseUtility.IsDvarEnabled( "ui_fakeDoubleXP" ) then
				...
				return CoD.DoubleXPUtility.MainModeHasDoubleXP( f17_arg0, f17_local0 and f17_local0[@"hash_EB7DDC7F079D51B"], f17_local2 )
			end
		end
	end
end
...
CoD.DoubleXPUtility.CurrentPlaylistHasMainMode = function ( f18_arg0, f18_arg1 )
	...
	if not IsPublicOrLeagueGame() and (not f18_local0 or f18_local0[@"LobbyMode"] == Enum[@"LobbyMode"][@"hash_3F5EE25D311E5223"]) and not CoD.DirectorUtility.ShowDirectorPregame( f18_arg0 ) then
		return false
	else
		...
		if f18_local1[@"id"] == LobbyData.GetLobbyMenuIDByName( LuaEnum.UI.DIRECTOR_ONLINE_MP_TRAINING ) or IsSimulateCT() then
			...
			local f18_local3 = f18_local2 and Engine[@"hash_63ACB99DBAD24D55"]( f18_local2 ) --[[ @ 0]]
...
CoD.DoubleXPUtility.CurrentPlaylistHasDoubleWeaponXP = function ( f19_arg0 )
	...
	if not IsPublicOrLeagueGame() and (not f19_local0 or f19_local0[@"LobbyMode"] == Enum[@"LobbyMode"][@"hash_3F5EE25D311E5223"]) and not CoD.DirectorUtility.ShowDirectorPregame( f19_arg0 ) then
		return false
	else
		...
		if f19_local1[@"id"] == LobbyData.GetLobbyMenuIDByName( LuaEnum.UI.DIRECTOR_ONLINE_MP_TRAINING ) or IsSimulateCT() then
			...
			local f19_local3 = f19_local2 and Engine[@"hash_63ACB99DBAD24D55"]( f19_local2 ) --[[ @ 0]]
			if f19_local3 and LuaUtils.PlaylistRulesIncludes( f19_local3.rules, @"scr_gunxpscalezm", "0" ) then
				return false
			else
				return CoD.DoubleXPUtility.MainModeHasDoubleWeaponXP( f19_arg0, f19_local0 and f19_local0[@"hash_EB7DDC7F079D51B"] )
			end
		end
	end
end
